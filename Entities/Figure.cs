namespace CourseWork89
{
    ...
    public class Figure
    {
        ...
        /// Отражение.
        /// </summary>
        public void Mirror(MouseEventArgs em, List<Vertex> vertices)
        {
            var s = new float[,]
            {
                {1, 0,  0},
            ...
            var dX = Math.Abs(vertices[1].X - vertices[0].X);
            var dY = Math.Abs(vertices[1].Y - vertices[0].Y);
            ...
            var e = new Vertex(vertices[0].X, vertices[1].Y);
            ToAndFromCenter(true, e);

            for (var i = 0; i < Vertexes.Count; i++)
                Vertexes[i] = Matrix_1x3_3x3(Vertexes[i], r);

            for (var i = 0; i < Vertexes.Count; i++)
                Vertexes[i] = Matrix_1x3_3x3(Vertexes[i], s);

            for (var i = 0; i < Vertexes.Count; i++)
                Vertexes[i] = Matrix_1x3_3x3(Vertexes[i], rNeg);
